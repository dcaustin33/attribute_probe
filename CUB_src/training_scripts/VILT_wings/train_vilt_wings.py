...
import torchvision
from models.vilt import ViLT_wings
...
def get_params(model: nn.Module, args):
    params = [ {"params": model.linear1.parameters(),"weight_decay": 0},
                {"params": model.classifier1.parameters(),"weight_decay": 0}]
    return params
...
def give_params_gradient(model):
    loss = 0
    for param in model.parameters():
        loss += torch.sum(param) * 0
    return loss
...
def training_step(data: dict, 
    ...
    prompts = [['This is a bird.'] * images.shape[0]][0]
    ...
    for i in range(2):
        ...
        loss += nn.BCEWithLogitsLoss()(output_logits, attributes.float())
        accurate, total = assess_accuracy(output_logits, attributes)    
        metrics['Accurate' + str(i + 1)] += accurate
        metrics['total'] += total / 2
    loss += give_params_gradient(model)
...
if __name__ == '__main__':
    ...
    val_metrics = {}
    val_metrics['total'] = 0
    val_metrics['Accurate1'] = 0
    val_metrics['Accurate2'] = 0


    model = ViLT_wings(args=None)
