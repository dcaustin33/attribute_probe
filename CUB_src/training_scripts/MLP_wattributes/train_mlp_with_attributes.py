...
def prepare_data(dataset_args, val_dataset_args):
    ...
    #create the dataset
    train_dataset = Cub2011_no_image(dataset_args['root'], dataset_args, download=download)
    ...
    val_dataset = Cub2011_no_image(dataset_args['root'], val_dataset_args, download=download, train = False)
...
def training_step(data: dict, 
    ...
    attributes, certainty, class_labels = data['attributes'].cuda(), data['certainty'].cuda(), data['class'].cuda()
    ...
    metrics['total'] += attributes.shape[0]
    metrics['class total'] += attributes.shape[0]
